...
class LSTM_Attention_simple(MergeLayer):
    ...
    def __init__(self, incoming,
                 num_units,
                 aln_num_units,
                 n_decodesteps,#decodr的最大长度
                 W_align=init.Normal(0.1),#init是from lasagne import layer导入的
                 U_align=init.Normal(0.1),
                 v_align=init.Normal(0.1),
                 nonlinearity_align=nonlinearities.tanh,#import lasagne.nonlinearities as nonlinearities
                 W_hid_to_ingate=init.Normal(0.1),
                 W_cell_to_ingate=init.Normal(0.1),
                 b_ingate=init.Constant(0.),
                 nonlinearity_ingate=nonlinearities.sigmoid,#ingate用的sigmoid
                 #W_in_to_forgetgate=init.Normal(0.1),
                 W_hid_to_forgetgate=init.Normal(0.1),
                 W_cell_to_forgetgate=init.Normal(0.1),
                 b_forgetgate=init.Constant(0.),#constant初始化为0，但是是0.！
